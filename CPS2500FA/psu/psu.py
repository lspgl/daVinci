...
class PSU:
    ...
    def setVoltage(self, voltage, verbose=False, getTime=False):
        ...
        if not getTime:
            v_ret_data = self.serial.voltage_and_recieve(
                self.adr, v_data, verbose=verbose)
        else:
            v_ret_data, t = self.serial.voltage_and_recieve(
                self.adr, v_data, getTime=getTime, verbose=verbose)
        ...
        if payload['adr'] is not None:
            ...
            if '1' in warn:
                for i, w in enumerate(warn):
                    if w == '1':
                        if verbose:
                            print(_C.RED + 'Warning: ' +
                                  _M.warning[i] + _C.ENDC)
            else:
                if verbose:
